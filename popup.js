...
function setBadge(label, type = "default") {
  ...
  elements.stateBadge.className = "pf-badge";
...
function renderPageInfo(pageInfo) {
  ...
  elements.previewMode.className = "pf-badge";
...
function collectYoutubePageInfo() {
  ...
  function getActiveTabName(pageUrl) {
    const selectedTabName = document.querySelector('yt-tab-shape[aria-selected="true"] .ytTabShapeTab')?.textContent?.trim();
  ...
  const title = singleUrl ? getVideoTitle() : cleanYoutubeTitle(readMetaContent('meta[property="og:title"]') || document.title);
